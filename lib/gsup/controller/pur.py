...
class PURController(GsupController):
    ...
    async def handle_message(self, peer, message):
        ...
        if imsi is None:
            ...
            response = GsupMessageBuilder.new().with_msg_type(MsgType.PURGE_MS_ERROR).build()
        ...
        try:
            ...
            response = GsupMessageBuilder.new().with_msg_type(MsgType.PURGE_MS_RESULT).with_ie('imsi', imsi).build()
            await self._send_gsup_response(peer, response)
        except Exception as e:
            await self._logger.logAsync(service='GSUP', level='ERROR', message=f"Error purging subscriber: {str(e)}")
            response = GsupMessageBuilder.new().with_msg_type(MsgType.PURGE_MS_ERROR).with_ie('imsi', imsi).build()
            await self._send_gsup_response(peer, response)